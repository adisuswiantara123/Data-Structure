...
// this version of binsort takes a comparison function
...
template<class T>
Chain<T>::~Chain()
{// Chain destructor. Delete all nodes in the chain.
...
{// Set x to the k'th element and delete it.
 // Throw OutOfBounds exception if no k'th element.
   ...
   // move p to k'th and remove it from the chain
   if (k == 1) // p is already at k'th
      first = first->link; // remove it
...
{// Insert x after the k'th element.
 // Throw OutOfBounds exception if no k'th element.
 // Pass NoMem exception if there's inadequate space.
   ...
   if (k) {// insert after p
           y->link = p->link;
           p->link = y;}
   else {// insert as the first element
         y->link = first;
         first = y;}
   return *this;
}
...
{// Insert the chain elements into the output stream.
   ChainNode<T> *current;
...
{// Sort using the bin sort method.
   ...
   // collect from bins into the sorted chain
   ChainNode<T> *y = 0;
   for (i = 0; i <= range; i++)
      if (bottom[i]) {// bin is not empty
        if (y) // not the first non-empty bin
          y->link = bottom[i];
        else // the first non-empty bin
          first = bottom[i];
        y = top[i];}
   if (y) y->link = 0;
