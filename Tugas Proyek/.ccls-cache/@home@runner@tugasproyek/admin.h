...
void Admin::simpanDataKeFilePasien() {
    ofstream file("data_pasien.txt", ios::app); // Use ios::app to append

    if (file.is_open()) {
        if (jumlahPasien > 0) {
            // Save only the last added data
            file << daftarPasien[jumlahPasien - 1].nama << " "
                 << daftarPasien[jumlahPasien - 1].usia << " "
                 << daftarPasien[jumlahPasien - 1].diagnosa << " "
                 << daftarPasien[jumlahPasien - 1].jenispasien << endl;
        }
...
void Admin::simpanDataKeFilePasien1() {
  ...
  if (file.is_open()) { 
    for (int i = 0; i < jumlahPasien; ++i) {
      file << daftarPasien[i].nama << " " << daftarPasien[i].usia << " "
           << daftarPasien[i].diagnosa << " " << daftarPasien[i].jenispasien
           << "  \n";
    }
